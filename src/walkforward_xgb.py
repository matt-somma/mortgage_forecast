...
OUTPUT_PATH = MODEL_RESULTS_DIR / "walk_forward_xgb.csv"


# Same features you already validated
DGS10_FEATURES = [
...
def build_models(train_df):
    ...
    dgs_model.fit(
        train_df[DGS10_FEATURES].astype(float),
        train_df["target_dgs10_chg_1w"].astype(float),
    )

    spread_model.fit(
        train_df[SPREAD_FEATURES].astype(float),
        train_df["target_spread_chg_1w"].astype(float),
    )
